...
%   Institution of Engineering and Technology.
    ...
    % General parameters 
    courant_factor                  = 1.5;                                  % A factor that determines duration of a time step within the CFL limit.
    number_of_cells_per_wavelength  = 20;                                   % A factor determining the accuracy limit of FDTD results.通过该参数确定源波形的傅里叶频谱中的最高频率，以达到一定的精度水平。
    ...
    lx                              = nxobj*dx;                             % Size of the original domain in x-direction.实际图形长度
...
% xlim([fdtd_domain.min_x fdtd_domain.max_x])
% ylim([fdtd_domain.min_y fdtd_domain.max_y])
...
% plot(pos_x,pos_x,'r')
% xlim([impressed_J(1).min_x impressed_J(1).max_x])
% ylim([impressed_J(1).min_y impressed_J(1).max_y])
    ...
    % x and y coordinates for epsr and Ez matrices  epsr和Ez矩阵的x和y坐标
    xcoor = round(linspace(fdtd_domain.min_x,fdtd_domain.max_x,nxp1),4);    % 4表示四舍五入到 4 位数；
    ...
    % initialize sinusoidal waveforms  %初始化正弦波形
